...
export function ReceiveForm({ selectedItem, onSave, onCancel, isEditing }: ReceiveFormProps) {
      ...
      if (selectedItem.documentId) {
        try {
          ...
          if (receiveData.receivingItems) {
            //   receiveData.receivingItems = receiveData.receivingItems.map((receiveItem) => ({
            //   ...receiveItem,
            //   amount: Number(receiveItem.quantity / (receiveItem.product.quantityIncluded)),
            //   internalId: receiveItem.product.internalId,
            //   description: receiveItem.product.description,
            //   quantityUomId: receiveItem.product.quantityUomId,
            //   caseUomId: receiveItem.product.caseUomId
            // }))
          ...
          <AccordionTrigger className="hover:no-underline">
            <div className="flex items-start gap-4 w-full">
              ...
              <div className="flex-1 text-left">
                <h4 className={item.qaInspectionStatusId=='INSPECTED'?'text-base badge-fullfilled':(item.qaInspectionStatusId=='PARTIALLY_INSPECTED'?'text-base badge-partiallyFulfilled': 'text-base badge-notFulfilled')}> {item.qaInspectionStatusId || "PENDING_INSPECTION"}</h4>
                <h4 className="text-base font-medium mt-1">{item.productName}</h4>
                <p className="text-sm text-gray-600">
                  {`${item.casesAccepted ? item.casesAccepted : ''} ${findUomName(item.caseUomId ?? '', packageType)} , ${item.quantityAccepted} ${findUomName(item.quantityUomId ?? '', weightUom)}`}
                </p>
              </div>
            </div>
          </AccordionTrigger>
              ...
              <Card className="p-4">
                <div className="grid grid-cols-1 gap-4 text-sm">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex-1">
                      <p className="font-medium whitespace-normal word-wrap break-word text-left">Ivan Song</p>
                    </div>
                  </div>
                  <div className="flex items-center justify-between">
                    <p className="text-gray-500 w-24 shrink-0 text-right pr-2">Notes:</p>
                    <div className="flex-1">
                      <p className="font-medium whitespace-normal word-wrap break-word text-left">Example notes that QA might write about the receiving order.</p>
                    </div>
                  </div>
                </div>
              </Card>
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col h-full">
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <div className="flex items-center gap-2">
                      <Badge className={field.value=='SUBMITTED'?'badge-page badge-fullfilled':'badge-page badge-notFulfilled'}>{field.value}</Badge>
                    </div>
                  </FormItem>
                )}
              />
        ...
        <div className="flex text-sm">
          <div className="flex flex-col justify-start space-x-2 p-4">
            ...
            <p className="font-medium">{`${totalCases || 0} ${totalCasesUnit}, ${totalQuantity || 0} ${totalWeightUnit}`}</p>
          </div>
        </div>
